...
$rec_date=date("Y-m-d");
		...
		$_POST[$school_code][10]="";
						...
						$sql_select = "select * from  achievement_main  where  test_type='2' and test_class='$_REQUEST[test_class]' and ed_year='$_REQUEST[ed_year]' and school='$school_code' ";
						...
						$data_num=mysqli_num_rows($dbquery_select);
									...
									officer='$officer',
									rec_date='$rec_date'
									where  test_type='2' and test_class='$_REQUEST[test_class]' and ed_year='$_REQUEST[ed_year]' and school='$school_code' ";
									$dbquery_update = mysqli_query($connect,$sql_update);
									}
									else {
									$sql_insert = "insert into achievement_main (test_type, test_class, ed_year, school, thai, math, science, score_avg, officer, rec_date) values ('2', '$_REQUEST[test_class]', '$_REQUEST[ed_year]', '$school_code', '$thai', '$math', '$science', '$score_avg', '$officer', '$rec_date')";
												if($score_avg>0){
...
echo "<tr align='center'>
	<td align=center><font color='#990000' size='3'><strong>บันทึกคะแนน NT $class_ar[$test_class]  ปีการศึกษา $_REQUEST[ed_year] </strong></font>
...
echo "<Td>โรงเรียน</Td><Td>ความสามารถด้านภาษา</Td><Td>ความสามารถด้านคำนวณ</Td><Td>ความสามารถด้านเหตุผล</Td><Td>เฉลี่ย</Td><Td></Td></Tr>";
$N=1;
$M=1;
While ($result = mysqli_fetch_array($dbquery))
	{
			...
			$color="#FFFFFF";
						...
						$sql_select = "select * from  achievement_main  where school='$result[school_code]' and ed_year='$_REQUEST[ed_year]' and test_class='$_REQUEST[test_class]'  and test_type='2' ";
						$dbquery_select = mysqli_query($connect,$sql_select);
...
if($result_select){
			...
			echo "<Td align='center'><a href=?option=achievement&task=main/add_score_2&index=5&ed_year=$_REQUEST[ed_year]&test_class=$_REQUEST[test_class]&edit_code=$result[school_code]><img src=images/edit.png border='0' alt='แก้ไข'></a></Td>";
			}
}
else{
echo "<Td align='center'><INPUT TYPE='button' name='smb' value='บันทึก' onclick='goto_url(1)' ></td>";
