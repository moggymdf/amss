...
<CENTER>
<h2>คะแนนสอบ NT และการประเมิน แบบที่ 1 </h2>
...
echo "</form>";
  ...
  //ส่วนของปีการศึกษา
 	$strQuery = "select distinct ed_year from achievement_main where  test_type='2' and test_class='$_REQUEST[class_index]' order by ed_year ";
...
echo "<Table width='85%' Border='0' Bgcolor='#Fcf9d8'>";
  echo "<Tr bgcolor='#FFCCCC'><Td  align='center' width='16%'>ปีการศึกษา</Td><Td  align='center' width='16%'>ความสามารถด้านภาษา</Td><td align='center'  width='16%'>ความสามารถด้านคำนวณ</td><Td align='center'  width='16%'>ความสามารถด้านเหตุผล</Td><Td align='center'  width='16%'>เฉลี่ย</Td><Td align='center' >ผลการประเมิน</Td></Tr>";
$M=1;
  for($x=1;$x<=$year_num;$x++){
	...
	$strQuery = "select thai, math, science, social, english, health, art, vocation, score_avg from achievement_main where test_type='2' and test_class='$_REQUEST[class_index]' and ed_year='$ed_year_ar[$x]' and school='$_REQUEST[school_code]' ";
			}
			$result = mysqli_query($connect,$strQuery);
		...
		if($score_avg>0){
		 echo "<Tr bgcolor=$color><Td  align='center' >$ed_year_ar[$x]</Td><Td  align='center'>$thai</Td><td align='center' >$math</td><Td align='center'>$science</Td><Td align='center' >$score_avg</Td><Td align='left' >";
		 //คะแนนจุดตัด
		$a=60;
		$b=30;
		$color[2]="#00CC00";
...
function goto_display(val){
	if(val==1){
		callfrm("?option=achievement&task=main/report4_1");
		}
}
</script>
