...
$strQuery = "select distinct ed_year from achievement_main where  test_type='3' and test_class='$_REQUEST[class_index]' and school='$_REQUEST[school_code]' order by ed_year desc limit 1";
}
...
//คะแนนสถานศึกษา
			$strQuery = "select thai, math, science, social, english, health, art, vocation, score_avg from achievement_main where test_type='3' and test_class='$_REQUEST[class_index]' and ed_year='$ed_year_ar[1]' and school='$_REQUEST[school_code]' ";
			$result = mysqli_query($connect,$strQuery);
			$ors = mysqli_fetch_array($result);
			if($ors['score_avg']>0){
