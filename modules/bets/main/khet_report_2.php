...
echo $school_ar[$_REQUEST['school_code']];
...
echo "<tr><td align='left'>จำนวนผู้สอบ : $student_num คน</td><td align='right'>[<a href=?option=bets&task=main/khet_report_2&page=$_REQUEST[page]&school_code=$_REQUEST[school_code]><<กลับ</a>]</td></tr>";
echo "</table>";
echo "<Table width='90%' Border='0' align='center'>";
...
echo "</table>";
	...
	// หาเกณฑ์
		...
		echo "<Tr align='center'><Td colspan='4' align='left'></Td><td colspan='6' align='right'>[<a href=?option=bets&task=main/khet_report_2&page=$_GET[page]&school_code=$_REQUEST[school_code]><<กลับ</a>]</td></Tr>";
		...
		echo "</td>";
		if($total_percent!=""){
		echo "<td align='center'><a href=?option=bets&task=main/khet_report_2&index=3&sch_test_id=$_GET[sch_test_id]&person_id=$result[person_id]&page=$_REQUEST[page]&school_code=$_REQUEST[school_code]><img src=images/browse.png border='0'></a></td>";
		}
		else{
		echo "<td></td>";
		}
...
echo "<Table width='90%' Border='0' align='center'>";
echo "<tr><td align='left'>*คำนวณเฉพาะข้อที่ผู้สอบทำข้อสอบเท่านั้น</td><td align='right'>[<a href=?option=bets&task=main/khet_report_2&index=2&sch_test_id=$_GET[sch_test_id]&page=$_REQUEST[page]&school_code=$_REQUEST[school_code]><<กลับ</a>]</td></tr>";
echo "</table>";
echo "<Table width='90%' Border='0' align='center'>";
...
$num_rows=0;
if($_REQUEST['school_code']!=""){
$sql = "select bets_sch_test.id from bets_sch_test left join bets_test on bets_sch_test.test_id=bets_test.id where bets_sch_test.school='$_REQUEST[school_code]' ";
$dbquery = mysqli_query($connect,$sql);
$num_rows=mysqli_num_rows($dbquery);
}

$pagelen=20; // กำหนดแถวต่อหน้า
$url_link="option=bets&task=main/khet_report_2&school_code=$_REQUEST[school_code]";
...
//**
if($_REQUEST['school_code']!=""){
$sql = "select *,bets_sch_test.id,bets_sch_test.test_id,bets_sch_test.rec_date from bets_sch_test left join bets_test on bets_sch_test.test_id=bets_test.id where bets_sch_test.school='$_REQUEST[school_code]' order by bets_sch_test.id limit $start,$pagelen";
$dbquery = mysqli_query($connect,$sql);
		echo  "<table width='85%' border='0' align='center'>";
		echo "<Tr bgcolor='#FFCCCC' align='center'><Td width='70'>ลำดับที่</Td><Td>ชื่อการสอบ</Td><Td width='100'>ชั้น</Td><Td width='100'>วดป<br>กำหนดการสอบ</Td><Td width='100'>คะแนน(%)</Td><Td width='100'>ผลประเมิน</Td><Td width='100'>ภาพรวม<br>สถานศึกษา</Td><Td width='100'>นักเรียน<br>รายบุคคล</Td></Tr>";
		...
		echo "<td align='center'>$rec_date</td><td>$total_percent</td><td>";
		if($total_percent<=$g2){
		$bgcolor="#FF0000";
		}
		else if($total_percent>$g2 and $total_percent<$g1){
		...
		echo "<td>";
		if($full_score>0){
		echo "<a href=?option=bets&task=main/khet_report_2&sch_test_id=$result[id]&test_id=$result[test_id]&index=1&page=$page&school_code=$_REQUEST[school_code]><img src=images/browse.png border='0'></a>";
		}
		echo "</td>";
		echo "<td>";
		if($full_score>0){
		echo "<a href=?option=bets&task=main/khet_report_2&sch_test_id=$result[id]&test_id=$result[test_id]&index=2&page=$page&school_code=$_REQUEST[school_code]><img src=images/admin/user.gif border='0'></a>";
		}
		echo "</td>";
		echo "</Tr>";
		$M++;
...
function goto_display(val){
	if(val==1){
		callfrm("?option=bets&task=main/khet_report_2");
		}
	else if(val==2){
		callfrm("?option=bets&task=main/khet_report_2&index=8");
		}
}
