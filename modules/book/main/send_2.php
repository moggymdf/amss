...
echo "<td width='514' colspan='3' align='left'>";
	...
	While ($result_workgroup = mysqli_fetch_array($dbquery_workgroup)){
			if($result_workgroup['workgroup']==$result_ref['workgroup']){
			echo  "&nbsp;&nbsp;<input type='radio'  name='workgroup' value='$result_workgroup[workgroup]' checked>&nbsp;$result_workgroup[workgroup_desc]<br>";
			}
			else{
...
if($_SESSION['login_status']<=4){
			if($_POST['sendto']=='all') {
			$sql_sendto = "select school_code from system_school  where  school_type='1' order by school_type,school_code";
