...
$sql = "update bookregister_receive set book_no='$_POST[book_no]', signdate='$_POST[signdate]', book_from='$_POST[book_from]', book_to='$_POST[book_to]', subject='$_POST[subject]', department='$_POST[group]', operation='$_POST[operation]', comment='$_POST[comment]' where ms_id='$_POST[id]' ";
$dbquery = mysqli_query($connect,$sql);
...
<table border="1" width="99%" id="table2" style="border-collapse: collapse" align="center">
				<tr bgcolor="#FFCCCC">
					...
					<font size="2" face="Tahoma">เลขทะเบียนรับ สพฐ.</font></td>
					<td align="center" width="50">
...
if($_REQUEST['search_index']==1){
		if($_REQUEST['department']!=""){
		$sql="select * from bookregister_receive where $_REQUEST[field] like '%$_REQUEST[search]%' and department='$_REQUEST[department]' order by year,register_number  limit $start,$pagelen ";
		}
		else{
...
if(($result['officer']==$user) and ($delete==1)){
echo "<Td align='center'><a href=?option=bookregister&task=main/receive&index=5&id=$id&page=$_REQUEST[page]&department=$_REQUEST[department]><img src=images/edit.png border='0' alt='แก้ไข'></a></Td>";
}
else{
echo "<td></td>";
}
