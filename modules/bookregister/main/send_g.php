...
<table border="1" width="99%" id="table2" style="border-collapse: collapse" align="center">
				<tr bgcolor="#99FFFF">
					...
					<font size="2" face="Tahoma">เลขทะเบียนส่ง กลุ่ม</font></td>
					<td align="center" width="50">
...
if($_REQUEST['search_index']==1){
		...
		$sql="select * from bookregister_send where $_REQUEST[field] like '%$_REQUEST[search]%' and sub_department='$sub_department_id'  order by year,register_number_g limit $start,$pagelen ";
		}
		else{
		$sql="select * from bookregister_send where $_REQUEST[field] like '%$_REQUEST[search]%' and sub_department='$sub_department_id' order by year,register_number_g limit $start,$pagelen ";
		}
		...
		if($_REQUEST['sub_department']!=""){
		$sql="select * from bookregister_send where $_REQUEST[field] like '%$_REQUEST[search]%' and sub_department='$sub_department' order by year,register_number_g  limit $start,$pagelen ";
		}
		else{
		$sql="select * from bookregister_send where $_REQUEST[field] like '%$_REQUEST[search]%'  and sub_department='$sub_department_id' order by year,register_number_g  limit $start,$pagelen ";
		}
...
$sql="select * from bookregister_send where  sub_department='$sub_department_id' order by year,register_number_g limit $start,$pagelen ";
...
echo "<Td align='center'><a href=?option=bookregister&task=main/send_g&index=2&id=$id&page=$_REQUEST[page]><img src=images/drop.png border='0' alt='ลบ'></a></Td>";
}
else{
echo "<td></td>";
}
if(($result['officer'])==$user and ($delete==1)){
echo "<Td align='center'><a href=?option=bookregister&task=main/send_g&index=5&id=$id&page=$_REQUEST[page]><img src=images/edit.png border='0' alt='แก้ไข'></a></Td>";
}
else{
echo "<td></td>";
}
...
echo "<td align='center'><a href=?option=book&task=main/send_g_2&id=$id&index=1><img src=images/next.gif border='0' alt='ส่งโรงเรียน'></td>";
}
else{
echo "<td></td>";
}
