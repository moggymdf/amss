...
if($index==1){
$rec_date = date("Y-m-d");
$sql = "insert into budget_receive (num, budget_year, book_number, out_date, book_ref, plan, project, activity, activity2, m_source, account, m_pay, item, detail, money, rec_date,officer)
 values ('$_POST[num]', '$year_active_result[budget_year]', '$_POST[book_number]','$_POST[out_date]', '$_POST[book_ref]','$_POST[plan]','$_POST[project]', '$_POST[activity]', '$_POST[activity2]', '$_POST[budget_m_source]', '$_POST[account]', '$_POST[m_pay]', '$_POST[item]', '$_POST[detail]', '$_POST[money]', '$rec_date', '$officer')";
$dbquery = mysqli_query($connect,$sql);
}
...
echo "<Tr bgcolor='#FFCCCC' align='center'><Td width='60'>ที่ใบงวด</Td><Td width='70'>วดป</Td><Td>รายการ</Td><Td width='120'>จำนวนเงิน</Td><Td width='50' align=center>รายละเอียด</td><Td width='50' align=center>File</td><td width='50' align='center'>ลบ</td><td width='50' align='center'>แก้ไข</Td><td width='50' align='center'>พิมพ์</Td><td width='60'>รวม</Td></Tr>";
...
While ($result = mysqli_fetch_array($dbquery))
	{
		...
		if($file!=""){
		echo "<Td><div align=center><a href=?option=budget&task=budget_unit/edit_upload_form&id=$id&page=$page><img src=./images/edit.png border='0' alt='แก้ไข File'></a></td>";
		}
		else{
		echo "<Td><div align=center><a href=?option=budget&task=budget_unit/receive_upload_form&id=$id&page=$page><img src=./images/icon/arrow_up.gif border='0' alt='Upload File'></a></td>";
		}
		if($officer==$result['officer']){
		...
		<Td><div align=center><a href=?option=budget&task=budget_unit/edit_receive_form&id=$id&page=$page><img src=./images/edit.png border='0' alt='แก้ไข'></a></Td><td><a href='modules/budget/budget_unit/receive_print.php?id=$id' target='_blank'><img src=images/b_print.png border='0' alt='print'></a></td>";
		}
		else{
		echo "<td>&nbsp;</td><td>&nbsp;</td><td><a href='modules/budget/budget_unit/receive_print.php?id=$id' target='_blank'><img src=images/b_print.png border='0' alt='print'></a></td>";
		}
...
echo "<Tr bgcolor='#FFCCCC'><Td colspan='2'></Td><Td align='center'>รวม</Td><Td align='center'>$receive_item_total</Td><Td colspan='6'></Td></Tr>";
echo "</Table>";
}
//จบส่วนแสดงผล
