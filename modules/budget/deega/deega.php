...
echo "<tr align='center'><td><font color='#006666' size='3'><strong>ทะเบียนขอเบิกเงินคงคลัง ปีงบประมาณ $year_active_result[budget_year]</strong></font></td></tr>";
echo "</table>";
}
...
echo "<Center>";
echo "<Font color='#006666' Size=3><B>ขอเบิกเงินคงคลัง ปีงบประมาณ $year_active_result[budget_year]</Font>";
echo "</Cener>";
echo "<Br><Br>";
...
echo "<Tr><Td ></Td><Td align='right' valign='top'>รายการขอเบิกตามฎีกานี้(แสดงเพียง 40 รายการ)&nbsp;&nbsp;</Td><Td align='left'>";
$sql_withdraw= "select budget_withdraw.id, budget_withdraw.item, budget_withdraw.money, person_main.name  from  budget_withdraw left join person_main on  budget_withdraw.officer=person_main.person_id where budget_withdraw.budget_year='$year_active_result[budget_year]'  and  (budget_withdraw.deega <1 or budget_withdraw.deega is null)  order by budget_withdraw.id desc
limit 40 ";
$dbquery_withdraw = mysqli_query($connect,$sql_withdraw);
$W=1;
While ($result_withdraw = mysqli_fetch_array($dbquery_withdraw)){
echo "<Input Type='checkbox' Name='withdraw_ck[$W]'  value='$result_withdraw[id]'>$result_withdraw[item]&nbsp;จำนวน&nbsp;$result_withdraw[money]&nbsp;บาท&nbsp;($result_withdraw[name]) ";
echo "<br>";
$W++;
}
echo "</Td></Tr>";
...
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=budget&task=deega/deega&index=3&id=$_GET[id]&page=$_REQUEST[page]&officer_index=$_REQUEST[officer_index]\"'>
		&nbsp;&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=budget&task=deega/deega&page=$_REQUEST[page]&officer_index=$_REQUEST[officer_index]\"'";
echo "</td></tr></table>";
}

//ส่วนลบข้อมูล
if($index==3){
$sql = "delete from budget_deega where id=$_GET[id]";
$dbquery = mysqli_query($connect,$sql);
}
...
if($index==4){
//เช็คเลขที่ฏีการซ้ำ
$sql= "select * from  budget_deega where budget_year='$year_active_result[budget_year]'  and deega_num='$_POST[deega_num]' ";
$dbquery = mysqli_query($connect,$sql);
$deega_number=mysqli_num_rows($dbquery);
			if($deega_number>=1){
			echo "<script>alert('เลขที่ฎีกาซ้ำ  ยกเลิกการดำเนินการ'); document.location.href='?option=budget&task=deega/deega&index=1';</script>\n";
			echo exit();
			}

$rec_date = date("Y-m-d");
$sql = "insert  into  budget_deega (budget_year, deega_num, doc, receive_num,  plan, project, activity, pay_group, item, withdraw, tax, pay, officer, rec_date)
values ('$year_active_result[budget_year]','$_POST[deega_num]', '$_POST[doc]', '$_POST[receive_num]', '$_POST[plan]', '$_POST[project]', '$_POST[activity]', '$_POST[pay_group]', '$_POST[item]', '$_POST[withdraw]', '$_POST[tax]', '$_POST[pay]', '$officer','$rec_date')";
$dbquery = mysqli_query($connect,$sql);

//	ส่วนของการบันทึกเลขที่ฎีกาในทะเบียนขอเบิก
for($x=1;$x<=40;$x++){
		if(isset($_POST['withdraw_ck'][$x])){
			if($_POST['withdraw_ck'][$x]>0){
			$withdraw_id=$_POST['withdraw_ck'][$x];
			$sql = "update budget_withdraw set deega='$_POST[deega_num]'  where  id='$withdraw_id' ";
			$dbquery = mysqli_query($connect,$sql);
			}
		}
}
...
if ($index==5){
$sql = "select * from  budget_deega where id='$_GET[id]' ";
$dbquery = mysqli_query($connect,$sql);
While ($result = mysqli_fetch_array($dbquery))
	{
		$id = $result['id'];
...
echo "<form id='frm1' name='frm1'>";
echo "<Center>";
echo "<Font color='#006666' Size=3><B>แก้ไขทะเบียนขอเบิกเงินคงคลัง ปีประมาณ$year_active_result[budget_year]</Font>";
echo "</Cener>";
echo "<Br><Br>";
echo "<Table width=70% Border=0 Bgcolor=#Fcf9d8>";
...
echo "<Tr><Td ></Td><Td align='right' valign='top'>รายการขอเบิกตามฎีกานี้(แสดงเพียง 40 รายการ)&nbsp;&nbsp;</Td><Td align='left'>";
$sql_withdraw= "select budget_withdraw.id, budget_withdraw.item, budget_withdraw.money, person_main.name  from  budget_withdraw left join person_main on  budget_withdraw.officer=person_main.person_id where budget_withdraw.budget_year='$year_active_result[budget_year]'  and  budget_withdraw.deega='$deega_num'  order by budget_withdraw.id ";
$dbquery_withdraw = mysqli_query($connect,$sql_withdraw);
$W=1;
While ($result_withdraw = mysqli_fetch_array($dbquery_withdraw)){
echo "<Input Type='checkbox' Name='withdraw_ck[$W]'  value='$result_withdraw[id]' checked>$result_withdraw[item]&nbsp;จำนวน&nbsp;$result_withdraw[money]&nbsp;บาท&nbsp;($result_withdraw[name]) ";
echo "<br>";
$W++;
}
...
echo "<Tr ><Td colspan='7' align='right'><INPUT TYPE='button' name='smb' value='<<กลับหน้าก่อน' onclick='location.href=\"?option=budget&task=deega/deega&page=$_GET[page]&officer_index=$_REQUEST[officer_index]\"'></Td></Tr>";
...
echo "<Tr><Td ></Td><Td align='right' valign='top'>รายการขอเบิกตามฎีกานี้&nbsp;&nbsp;</Td><Td align='left'>";
$sql_withdraw= "select budget_withdraw.id, budget_withdraw.item, budget_withdraw.money, person_main.name  from  budget_withdraw left join person_main on  budget_withdraw.officer=person_main.person_id where budget_withdraw.budget_year='$year_active_result[budget_year]'  and  budget_withdraw.deega='$deega_num' order by budget_withdraw.id" ;
$dbquery_withdraw = mysqli_query($connect,$sql_withdraw);
$W=1;
While ($result_withdraw = mysqli_fetch_array($dbquery_withdraw)){
echo "<Input Type='checkbox' Name='withdraw[$W]'  value='$result_withdraw[id]' checked>$result_withdraw[item]&nbsp;จำนวน&nbsp;$result_withdraw[money]&nbsp;บาท&nbsp;($result_withdraw[name]) ";
echo "<br>";
$W++;
}
echo "</Td></Tr>";
...
$sql = "select * from  budget_deega where budget_year='$year_active_result[budget_year]'  and officer='$officer' order by id ";
}
else{
$sql = "select * from  budget_deega where budget_year='$year_active_result[budget_year]' order by id ";
}
$dbquery = mysqli_query($connect,$sql);
...
$url_link="option=budget&task=deega/deega&officer_index=$_REQUEST[officer_index]";  // 2_กำหนดลิงค์ฺ
...
if($_REQUEST['officer_index']==1){
$sql = "select * from  budget_deega where budget_year='$year_active_result[budget_year]' and officer='$officer' order by id limit $start,$pagelen";
}
else{
$sql = "select * from  budget_deega where budget_year='$year_active_result[budget_year]' order by id limit $start,$pagelen";
}
$dbquery = mysqli_query($connect,$sql);

echo "<Tr><Td colspan='5' align='left'><INPUT TYPE='button' name='smb' value='ลงทะเบียน' onclick='location.href=\"?option=budget&task=deega/deega&index=1\"'></Td>";
if($_REQUEST['officer_index']==1){
echo "<Td colspan='5' align='right'><INPUT TYPE='button' name='smb' value='ทั้งหมด[คลิก]' onclick='location.href=\"?option=budget&task=deega/deega\"'></Td>";
}
else{
echo "<Td colspan='5' align='right'><INPUT TYPE='button' name='smb' value='เฉพาะเจ้าหน้าที่นี้[คลิก]' onclick='location.href=\"?option=budget&task=deega/deega&officer_index=1\"'></Td>";
}
echo "</Tr>";
echo "<Tr bgcolor='#FFCCCC' align='center'><Td width='50'>ที่</Td><Td width='70'>ว/ด/ป</Td><Td width='70'>เลขที่ฎีกา</Td><Td width='100'>เลขที่เอกสาร</Td><Td width='70'>ใบงวด</Td><Td>รายการ</Td><Td width='90'>ขอเบิก</Td><Td width='90'>ภาษี</Td><Td width='90'>รับจริง</Td><Td></Td></Tr>";
...
While ($result = mysqli_fetch_array($dbquery))
	...
	$to_date=$rec_day.$t_month[$rec_month].$t_year;
		...
		echo "<Tr bgcolor='$color' align='center'><Td>$N</Td><Td>$to_date</Td><Td>$deega_num</Td><Td align=left>$doc</Td><Td>$receive_num</Td>";
echo "<Td align=left>$item</Td>";
echo "<Td align=right>$withdraw</Td><Td align=right>$tax</Td><Td align=right>$pay</Td>
		<Td width='70' align='left'>&nbsp;&nbsp;<a href=?option=budget&task=deega/deega&id=$id&index=7&page=$page&officer_index=$_REQUEST[officer_index]><img src=./images/browse.png border='0' alt='รายละเอียด'></a>&nbsp;";
if($officer==$result['officer']){
echo "<a href=?option=budget&task=deega/deega&id=$id&index=2&page=$page&officer_index=$_REQUEST[officer_index]><img src=./images/drop.png border='0' alt='ลบ'></a>&nbsp;<a href=?option=budget&task=deega/deega&id=$id&index=5&page=$page&officer_index=$_REQUEST[officer_index]><img src=./images/edit.png border='0' alt='แก้ไข'></a>";
}
echo "</Td>	</Tr>";
$M++;
...
function goto_url(val){
	if(val==0){
		callfrm("?option=budget&task=deega/deega");   // page ย้อนกลับ
	}else if(val==1){
		if(frm1.deega_num.value == ""){
			alert("กรุณากรอกเลขทีฎีกา");
		}else if(frm1.receive_num.value==""){
			...
			callfrm("?option=budget&task=deega/deega&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url_update(val){
	if(val==0){
		...
		if(frm1.deega_num.value == ""){
			...
			callfrm("?option=budget&task=deega/deega&index=6");   //page ประมวลผล
		}
	}
}
