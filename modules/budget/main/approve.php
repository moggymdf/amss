...
echo "<tr align='center'><td><font color='#006666' size='3'><strong>อนุมัติจ่ายเงิน ปีงบประมาณ$year_active_result[budget_year]</strong></font></td></tr>";
echo "</table>";
}
...
echo "<Center>";
echo "<Font color='#006666' Size=3><B>แก้ไขการอนุมัติจ่ายเงิน ปีงบประมาณ$year_active_result[budget_year]</B></Font>";
echo "</Cener>";
echo "<Br><Br>";
...
if ($index==6){
$rec_date = date("Y-m-d");
$sql = "update budget_main set approve='$_POST[approve]',
approve_name='$officer', approve_date='$rec_date' where id='$_POST[id]'";
$dbquery = mysqli_query($connect,$sql);
}
...
if ($index==6.5){
$rec_date = date("Y-m-d");
$sql = "update budget_main set approve='1', approve_date='$rec_date', approve_name='$officer' where id='$_GET[id]'";
$dbquery = mysqli_query($connect,$sql);
}
...
echo "<Table width='70%' Border='0'>";
echo "<Tr ><Td colspan='11' align='right'><INPUT TYPE='button' name='smb' value='<<กลับหน้าก่อน' onclick='location.href=\"?option=budget&task=main/approve&page=$_GET[page]\"'></Td></Tr>";
...
$url_link="option=budget&task=main/approve";
...
echo "<Tr bgcolor='#FFCCCC' align='center'><Td width='30'>ที่</Td><Td width='70'>วดป</Td><Td width='70'>ที่เอกสาร</Td><Td width='75'>อ้างอิงขอเบิก</Td><Td width='75'>อ้างอิงฎีกา</Td><Td>รายการ</Td><Td width='110'>จำนวนเงิน</Td><Td width='110'>ประเภทเงิน</Td><Td width='60'>รายละเอียด</Td><Td width='60'>อนุมัติ</Td><Td width='60'>แก้ไข</Td></Tr>";
...
$t_year=($rec_year+543)-2500;
		...
		$approve_pic="<a href=?option=budget&task=main/approve&id=$id&index=6.5&page=$page><img src=./images/yellow.gif border='0' alt='คลิกอนุมัติจ่ายเงิน'></a>";
		}
...
if($refer_wd_id>0){
echo "<a href=?option=budget&task=main/refer_wd&id=$refer_wd_id&page=$page&back_page=approve>$refer_wd_id</a>";
}
echo "</Td>";

echo "<td align='center'>";
if($refer_deega_id>0){
echo "<a href=?option=budget&task=main/refer_deega&id=$refer_deega_id&page=$page&back_page=approve>$refer_deega_id</a>";
}
echo "</td>";

echo "<Td align=left>$item</Td><Td align='right'>$pay_amount</Td><Td align='left'>$type_bud_text</Td><Td align='center'><a href=?option=budget&task=main/approve&index=7&id=$id&page=$page><img src=./images/browse.png border='0' alt='รายละเอียด'></a></Td><td align='center'>$approve_pic</td>";
echo "<Td align='center'><a href=?option=budget&task=main/approve&index=5&id=$id&page=$page><img src=images/edit.png border='0' alt='แก้ไข'></a></div></Td></Tr>";
$M++;
...
echo "<tr><td><img src=./images/green.gif></td><td>อนุมัติให้จ่่ายเงินได้</td></tr>";
echo "<tr><td><img src=./images/red.gif></td><td>ไม่อนุมัติ</td></tr>";
echo "</Table>";
}
...
function goto_url_update(val){
	if(val==0){
		callfrm("?option=budget&task=main/approve");   // page ย้อนกลับ
	}else if(val==1){
	callfrm("?option=budget&task=main/approve&index=6");   //page ประมวลผล
	}
}
