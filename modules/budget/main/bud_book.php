...
echo "<font color='#006666' size='3'><strong>รายงานเงินงบประมาณ</strong></font><br />";
...
echo "หน้า	";
			for($i=1; $i<=$totalpages; $i++)	{
					...
					echo "<a href=$PHP_SELF?$url_link&page=$i&year_index=$_REQUEST[year_index]>[$i]</a>";
					}
			}
echo "</div>";
}
if($totalpages>15){
			...
			if($page!=1){
			$f_page1=$page-1;
			echo "<<a href=$PHP_SELF?$url_link&page=1&year_index=$_REQUEST[year_index]>หน้าแรก </a>";
			echo "<<<a href=$PHP_SELF?$url_link&page=$f_page1&year_index=$_REQUEST[year_index]>หน้าก่อน </a>";
			}
			else {
			echo "หน้า	";
			}
			...
			if($page<$totalpages)	{
			$f_page2=$page+1;
			echo "<a href=$PHP_SELF?$url_link&page=$f_page2&year_index=$_REQUEST[year_index]> หน้าถัดไป</a>>>";
			echo "<a href=$PHP_SELF?$url_link&page=$totalpages&year_index=$_REQUEST[year_index]> หน้าสุดท้าย</a>>";
			}
			echo " <select onchange=\"location.href=this.options[this.selectedIndex].value;\" size=\"1\" name=\"select\">";
			echo "<option  value=\"\">หน้า</option>";
				for($p=1;$p<=$totalpages;$p++){
				echo "<option  value=\"?$url_link&page=$p&year_index=$_REQUEST[year_index]\">$p</option>";
				}
			echo "</select>";
echo "</div>";
}
...
$office_ar_display=number_format($office_ar[$id_num],2);
		...
		echo "<Td align=center><a href=?option=budget&task=main/bud_book&page=$page&year_index=$_REQUEST[year_index]>$tungnee</a></Td>	</Tr>";
		break; //ออกจากloop
		}
		else {
		echo "<Td align=center><a href=?option=budget&task=main/bud_book&cal_id=$id_num&page=$page&year_index=$_REQUEST[year_index]>ถึงนี้</a></Td>	</Tr>";
		}
