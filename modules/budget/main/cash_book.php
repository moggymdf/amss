...
if($_REQUEST['type_id_index']!=""){
		...
		else if($_REQUEST['type_id_index']==2){
		$sql = "select  * from budget_main where budget_year='$year_active_result[budget_year]' and type_id='200' and status!='null' order by rec_date, id";
		}
		else if($_REQUEST['type_id_index']==3){
		$sql = "select  * from budget_main where budget_year='$year_active_result[budget_year]' and type_id>='300' and status!='null' order by rec_date, id";
		}
}
else {
$sql = "select  * from budget_main where budget_year='$year_active_result[budget_year]' and status!='null' order by rec_date, id";
}
...
echo "</select>&nbsp;";
		...
		echo  "<option value=2 $type_select2>เงินงบประมาณ</option>";
		echo  "<option value=1 $type_select1>เงินนอกงบประมาณ</option>";
		echo  "<option value=3 $type_select3>เงินรายได้แผ่นดิน</option>";
		echo "</select>";
...
$sql= "select * from  budget_main where  budget_year='$year_active_result[budget_year]' and status!='null' order by rec_date, id  limit $start,$pagelen";
}
$dbquery = mysqli_query($connect,$sql);
...
$office_ar_display=number_format($office_ar[$id_num],2);
		...
		echo "<Td align=center><a href=?option=budget&task=main/cash_book&cal_id=$id_num&page=$page&type_id_index=$_REQUEST[type_id_index]&year_index=$_REQUEST[year_index]>ถึงนี้</a></Td>	</Tr>";
		}
