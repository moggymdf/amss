...
echo "<tr align='center'><td><font color='#006666' size='3'><strong>ทะเบียนการเปลี่ยนแปลงสถานะเงินงบประมาณ ปีงบประมาณ$year_active_result[budget_year]</strong></font></td></tr>";
echo "</table>";
}
...
echo "<Center>";
echo "<Font color='#006666' Size=3><B>เปลี่ยนแปลงสถานะเงินงบประมาณ ปีงบประมาณ$year_active_result[budget_year]</Font>";
echo "</Cener>";
echo "<Br><Br>";
...
echo "<Tr align='left'><Td width=20></Td><Td align='right'>จำนวนเงิน</Td><Td><Input Type='Text' Name='change_amount' id='change_amount' Size='15' onkeydown='digitOnly()'>&nbsp;บาท</Td></Tr>";
...
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=budget&task=main/change_bud&index=3&id=$_GET[id]&page=$_REQUEST[page]\"'>
		&nbsp;&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=budget&task=main/change_bud&page=$_REQUEST[page]\"'";
echo "</td></tr></table>";
}
...
if($index==4){
$rec_date = date("Y-m-d");
$sql = "insert into budget_main (budget_year, doc, type_id, item, change_amount, status, rec_date, officer) values ('$year_active_result[budget_year]', '$_POST[doc]', '200', '$_POST[item]','$_POST[change_amount]', '$_POST[status]', '$rec_date','$officer')";
$dbquery = mysqli_query($connect,$sql);
}
...
echo "<form id='frm1' name='frm1'>";
echo "<Center>";
echo "<Font color='#006666' Size=3><B>แก้ไขการเปลี่ยนแปลงสถานะเงินงบประมาณ ปีงบประมาณ$year_active_result[budget_year]</B></Font>";
echo "</Cener>";
echo "<Br><Br>";
...
echo "<Tr align='left'><Td width=20></Td><Td align='right'>จำนวนเงิน</Td><Td><Input Type='Text' Name='change_amount' id='change_amount' Size='15'  value='$ref_result[change_amount]' onkeydown='digitOnly()'>&nbsp;บาท</Td></Tr>";
echo "</Table>";
...
$sql = "update budget_main set doc='$_POST[doc]', item='$_POST[item]', change_amount='$_POST[change_amount]', status='$_POST[status]', officer='$officer', rec_date='$rec_date' where id='$_POST[id]'";
$dbquery = mysqli_query($connect,$sql);
}
...
if ($index==7){
echo "<Center>";
echo "<Font color='#006666' Size=3><B>รายละเอียดการเปลี่ยนแปลงสถานะเงินงบประมาณ ปีงบประมาณ$year_active_result[budget_year]</B></Font>";
echo "</Cener>";
echo "<Br><Br>";
echo "<Table   width=60% Border=0 Bgcolor=#Fcf9d8>";
echo "<Tr ><Td colspan='11' align='right'><INPUT TYPE='button' name='smb' value='<<กลับหน้าก่อน' onclick='location.href=\"?option=budget&task=main/change_bud&page=$_GET[page]&type_id_index=$_REQUEST[type_id_index]\"'></Td></Tr>";
$sql = "select * from budget_main where id='$_GET[id]'";
$dbquery = mysqli_query($connect,$sql);
$ref_result = mysqli_fetch_array($dbquery);
list($rec_year,$rec_month,$rec_day) = explode("-",$ref_result['rec_date']);
...
echo "<Tr align='left'><Td align='right'>จำนวนเงิน&nbsp;&nbsp;</Td><Td><Input Type='Text' Name='change_amount' id='change_amount' Size='15'  value='$ref_result[change_amount]' readonly>&nbsp;บาท</Td></Tr>";
...
$url_link="option=budget&task=main/change_bud";  // 2_กำหนดลิงค์ฺ
...
$sql = "select * from budget_main where budget_year='$year_active_result[budget_year]' and type_id='200' and status between '5' and '10' order by rec_date, id limit $start,$pagelen";
$dbquery = mysqli_query($connect,$sql);
echo  "<table width=95% border=0 align=center>";
echo "<Tr><Td colspan='9' align='left'><INPUT TYPE='button' name='smb' value='เพิ่มรายการ' onclick='location.href=\"?option=budget&task=main/change_bud&index=1\"'></Td></Tr>";

echo "<Tr bgcolor=#FFCCCC align='center'><Td width='30'>ที่</Td><Td width='70'>วดป</Td><Td width='100'>ที่เอกสาร</Td><Td>รายการ</Td><Td width='250'>ลักษณะรายการ</Td><Td width='120'>จำนวนเงิน</Td><Td width='80'>รายละเอียด</Td><Td width='40'>ลบ</Td><Td width='40'>แก้ไข</Td></Tr>";
...
$t_year=($rec_year+543)-2500;
		...
		echo "<Tr bgcolor=$color><Td align='center'>$N</Td> <Td>$rec_day $t_month[$rec_month] $t_year</Td><Td align=left>$doc</Td><Td align=left>$item</Td><Td>$budget_status[$status]</Td><Td align='right'>$change_amount</Td><Td align='center'><a href=?option=budget&task=main/change_bud&index=7&id=$id&page=$page&type_id_index=$_REQUEST[type_id_index]><img src=./images/browse.png border='0' alt='รายละเอียด'></a></Td>";
		if($officer==$result['officer']){
		echo "<Td><div align='center'><a href=?option=budget&task=main/change_bud&index=2&id=$id&page=$page><img src=images/drop.png border='0' alt='ลบ'></a></div></Td>
		<Td align='center'><a href=?option=budget&task=main/change_bud&index=5&id=$id&page=$page><img src=images/edit.png border='0' alt='แก้ไข'></a></div></Td>
	</Tr>";
		}
		else{
		echo "<td>&nbsp;</td><td>&nbsp;</td></Tr>";
...
function goto_url(val){
	if(val==0){
		callfrm("?option=budget&task=main/change_bud");   // page ย้อนกลับ
	}else if(val==1){
		if(frm1.doc.value == ""){
			...
			callfrm("?option=budget&task=main/change_bud&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url_update(val){
	if(val==0){
		...
		if(frm1.update_date.value == ""){
			...
			callfrm("?option=budget&task=main/change_bud&index=6");   //page ประมวลผล
		}
	}
}
</script>
