...
$sql = "select  * from budget_main where budget_year='$year_active_result[budget_year]' and type_id='$_REQUEST[type_id_index]' and status!='null' order by rec_date, id";
}
else {
...
if($_REQUEST['type_id_index']!=""){
$sql= "select * from  budget_main where  budget_year='$year_active_result[budget_year]' and type_id='$_REQUEST[type_id_index]' and status!='null' order by rec_date, id  limit $start,$pagelen";
}
else{
$sql= "select * from  budget_main where  budget_year='$year_active_result[budget_year]' and type_id<'200' and status!='null' order by rec_date, id  limit $start,$pagelen";
}
$dbquery = mysqli_query($connect,$sql);
...
$office_ar_display=number_format($office_ar[$id_num],2);
		...
		echo "<Td align=center><a href=?option=budget&task=main/ex_bud_book&page=$page&type_id_index=$_REQUEST[type_id_index]&year_index=$_REQUEST[year_index]>$tungnee</a></Td>	</Tr>";
		break; //ออกจากloop
		}
		else {
		echo "<Td align=center><a href=?option=budget&task=main/ex_bud_book&cal_id=$id_num&page=$page&type_id_index=$_REQUEST[type_id_index]&year_index=$_REQUEST[year_index]>ถึงนี้</a></Td>	</Tr>";
		}
...
if(isset($receive_total_ar[$id_num])){
$receive_total_ar_display=number_format($receive_total_ar[$id_num],2);
}
else{
$receive_total_ar_display="";
}

if(isset($pay_total_ar[$id_num])){
$pay_total_ar_display=number_format($pay_total_ar[$id_num],2);
}
else{
$pay_total_ar_display="";
}

if(!isset($cash_ar_display)){
$cash_ar_display="";
}

if(!isset($bank_ar_display)){
$bank_ar_display="";
}

if(!isset($office_ar_display)){
$office_ar_display="";
}
