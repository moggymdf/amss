...
$sql = "select  * from budget_main where budget_year='$year_active_result[budget_year]' and type_id between '300' and '399' and status!='null' order by rec_date, id";
}
$dbquery = mysqli_query($connect,$sql);
...
$sql= "select * from  budget_main where  budget_year='$year_active_result[budget_year]' and type_id between '300' and '399' and status!='null' order by rec_date, id  limit $start,$pagelen";
}
$dbquery = mysqli_query($connect,$sql);
...
$office_ar_display=number_format($office_ar[$id_num],2);
		...
		echo "<Td align=center><a href=?option=budget&task=main/income_bud_book&page=$page&type_id_index=$_REQUEST[type_id_index]&year_index=$_REQUEST[year_index]>$tungnee</a></Td>	</Tr>";
		break; //ออกจากloop
		}
		else {
		echo "<Td align=center><a href=?option=budget&task=main/income_bud_book&cal_id=$id_num&page=$page&type_id_index=$_REQUEST[type_id_index]&year_index=$_REQUEST[year_index]>ถึงนี้</a></Td>	</Tr>";
		}
