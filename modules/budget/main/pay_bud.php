...
echo "<tr align='center'><td><font color='#006666' size='3'><strong>ทะเบียนสั่งจ่ายเงินงบประมาณ ปีงบประมาณ$year_active_result[budget_year]</strong></font></td></tr>";
echo "</table>";
}
...
echo "<Center>";
echo "<Font color='#006666' Size=3><B>เพิ่มข้อมูลสั่งจ่ายเงินงบประมาณ ปีงบประมาณ$year_active_result[budget_year]</Font>";
echo "</Cener>";
echo "<Br><Br>";
...
$item = $result['item'];
$item = substr($item,0,150);
echo  "<option value =$deega_num>$deega_num $item</option>";
	}
echo "</select>";
echo "</div></td></tr>";
...
echo "<Tr align='left'><Td align='right'>งบรายจ่าย&nbsp;&nbsp;</Td>";
...
if($index==4){
$rec_date = date("Y-m-d");
$sql = "insert into budget_main (budget_year, doc, refer_wd_id, refer_deega_id, type_id, item, pay_group, pay_amount, payed_person, rec_date, officer) values ('$year_active_result[budget_year]', '$_POST[doc]','$_POST[refer_wd_id]', '$_POST[refer_deega_id]','200', '$_POST[item]', '$_POST[pay_group]','$_POST[pay_amount]', '$_POST[payed_person]', '$rec_date','$officer')";
$dbquery = mysqli_query($connect,$sql);
}
...
echo "<form id='frm1' name='frm1'>";
echo "<Center>";
echo "<Font color='#006666' Size=3><B>แก้ไขทะเบียนสั่งจ่ายเงินงบประมาณ ปีงบประมาณ$year_active_result[budget_year]</B></Font>";
echo "</Cener>";
echo "<Br><Br>";
...
$item = substr($item,0,150);
		...
		echo  "<option value = $deega_num selected>$deega_num $item</option>";
		}
		else{
		echo  "<option value = $deega_num>$deega_num $item</option>";
		}
...
refer_deega_id='$_POST[refer_deega_id]',
item='$_POST[item]',
pay_amount='$_POST[pay_amount]',
payed_person='$_POST[payed_person]',
pay_group='$_POST[pay_group]', officer='$officer' , rec_date='$rec_date' where id='$_POST[id]'";
$dbquery = mysqli_query($connect,$sql);
}
...
echo "<form id='frm1' name='frm1'>";
echo "<Center>";
echo "<Font color='#006666' Size=3><B>รายละเอียดสั่งจ่ายเงินงบประมาณ ปีงบประมาณ$year_active_result[budget_year]</B></Font>";
echo "</Cener>";
echo "<Br><Br>";

echo "<Table   width='60%' Border='0' Bgcolor='#Fcf9d8'>";
echo "<Tr ><Td colspan='11' align='right'><INPUT TYPE='button' name='smb' value='<<กลับหน้าก่อน' onclick='location.href=\"?option=budget&task=main/pay_bud&page=$_GET[page]\"'></Td></Tr>";
...
//ส่วนของการคำนวณ
$pay_total=0; //รวมจ่าย
$sql = "select id, pay_amount from  budget_main where budget_year='$year_active_result[budget_year]' and pay_amount !='null' and  type_id='200' order by rec_date, id";
$dbquery = mysqli_query($connect,$sql);
...
$url_link="option=budget&task=main/pay_bud";
...
//จบแยกหน้า
		...
		echo "<Tr><Td align='left'><INPUT TYPE='button' name='smb' value='เพิ่มรายการสั่งจ่าย' onclick='location.href=\"?option=budget&task=main/pay_bud&index=1\"'></Td>";
echo "</Tr></Table>";

$sql = "select * from  budget_main where budget_year='$year_active_result[budget_year]' and pay_amount !='null' and  type_id='200' order by rec_date, id limit $start,$pagelen";

$dbquery = mysqli_query($connect,$sql);
$num_effect = mysqli_num_rows($dbquery );  // จำนวนข้อมูลในหน้านี้

echo  "<table width=95% border=0 align=center>";
echo "<Tr bgcolor='#FFCCCC' align='center'><Td width='30'>ที่</Td><Td width='70'>วดป</Td><Td width='100'>ที่เอกสาร</Td><Td width='75'>อ้างอิงขอเบิก</Td><Td width='75'>อ้างอิงฎีกา</Td><Td>รายการ</Td><Td width='100'>จำนวนเงิน</Td><Td width='50'>รายละเอียด</Td><Td width='40'>ลบ</Td><Td width='40'>แก้ไข</Td><Td width='50'>พิมพ์ใบสั่งจ่าย</Td><Td width='60'>รวม</Td></Tr>";
...
if($refer_wd_id>0){
echo "<a href=?option=budget&task=main/refer_wd&id=$refer_wd_id&page=$page&back_page=pay_bud>$refer_wd_id</a>";
}
echo "</Td>";

echo "<td align='center'>";
if($refer_deega_id>0){
echo "<a href=?option=budget&task=main/refer_deega&id=$refer_deega_id&page=$page&back_page=pay_bud>$refer_deega_id</a>";
}
echo "</td>";

echo "<Td align=left>$item</Td><Td align='right'>$pay_amount</Td><Td align='center'><a href=?option=budget&task=main/pay_bud&index=7&id=$id&page=$page><img src=./images/browse.png border='0' alt='รายละเอียด'></a></Td>";
		if(($officer==$result['officer']) and !(($result['approve']!=0) or ($result['check_number']!=""))){
echo "<Td align='center'><a href=?option=budget&task=main/pay_bud&index=2&id=$id&page=$page><img src=images/drop.png border='0' alt='ลบ'></a></Td>
		<Td align='center'><a href=?option=budget&task=main/pay_bud&index=5&id=$id&page=$page><img src=images/edit.png border='0' alt='แก้ไข'></a></div></Td>";
		}
		else{
		echo "<td>&nbsp;</td><td>&nbsp;</td>";
		}
echo "<td align='center'><a href='modules/budget/main/form1.php?id=$id' target='_blank'><img src=images/b_print.png border='0' alt='print'></a></td>";
		...
		if($_GET['cal_id']==$id){
		echo "<Td align=center><a href=?option=budget&task=main/pay_bud&page=$page>$tungnee</a></Td>	</Tr>";
		break; //ออกจากloop
		}
		else {
		echo "<Td align=center><a href=?option=budget&task=main/pay_bud&cal_id=$id&page=$page>ถึงนี้</a></Td>	</Tr>";
		}
	//ออกจากloop
	if(!isset($_GET['id'])){
	$_GET['id']="";
...
echo "<Tr bgcolor=#FFCCCC><Td></Td><Td></Td><Td></Td><Td></Td><Td></Td><Td align='center'>รวม</Td><Td align='center'>$pay_item_total</Td><Td></Td><Td></Td><Td></Td><Td></Td><Td></Td></Tr>";
