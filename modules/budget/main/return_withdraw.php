...
echo "<Tr><Td ></Td><Td></Td><Td align='left'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<Input Type='checkbox' Name='check' value='1' checked>&nbsp;บันทึกข้อมูลในทะเบียนรับเงินงบประมาณด้วย</Td></Tr>";
...
$_POST['check']="";
		...
		if($_POST['check']==1){
		$sql = "insert into budget_main (budget_year, doc, type_id, item, receive_amount, status,  rec_date, officer) values ('$year_active_result[budget_year]', 'คืนเิงิน$_POST[doc]', '200', '$_POST[item]','$_POST[amount]', '1', '$rec_date','$officer')";
...
$t_year=($rec_year+543)-2500;
		...
		echo "<Tr bgcolor='$color' align='center'><Td >$N</Td><Td width='100' align='left'>$rec_day $t_month[$rec_month] $t_year</Td><Td align='left'>$document</Td><Td align=left>";
...
function goto_url_update(val){
	if(val==0){
		callfrm("?option=budget&task=main/return_withdraw&page=<?=$_GET[page];?>");   // page ย้อนกลับ
	}else if(val==1){
		if(frm1.doc.value == ""){
			alert("กรุณากรอกที่เอกสาร");
		}else if(frm1.item.value==""){
			...
			callfrm("?option=budget&task=main/return_withdraw&index=6&page=<?=$_GET[page];?>");   //page ประมวลผล
		}
	}
}
