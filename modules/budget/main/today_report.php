...
if(!$_POST){
$sql = "select * from budget_main where budget_year='$year_active_result[budget_year]' and type_id='200' and status!='null' order by rec_date, id";
}
else{
$select_year=$_POST['select_year']-543;
$date_index=$select_year."-".$_POST['select_month']."-".$_POST['select_date'];

$sql = "select * from budget_main where budget_year='$year_active_result[budget_year]' and type_id='200' and status!='null' and rec_date<='$date_index' order by rec_date, id";
}
			$dbquery = mysqli_query($connect,$sql);
...
$cash_ar[200]=$cash_tank;
$bank_ar[200]=$bank_tank;
$office_ar[200]=$office_tank;
$sum_type[200]=$cash_ar[200]+$bank_ar[200]+$office_ar[200]; //รวมเงินในแต่ละรายการ
$total_cash=$total_cash+$cash_ar[200];
$total_bank=$total_bank+$bank_ar[200];
$total_office=$total_office+$office_ar[200];
////จบส่วนคำนวณเงินงบประมาณ/////

$sql_ex_bud = "select  * from  budget_type where budget_year='$year_active_result[budget_year]' order by category_id,type_id";
...
$office_tank=0;	 //เงินฝากส่วนราชการ
if(!$_POST){
$sql = "select * from budget_main where budget_year='$year_active_result[budget_year]' and type_id='$result_ex_bud[type_id]' and status!='null' order by rec_date, id";
}
else{
$select_year=$_POST['select_year']-543;
$date_index=$select_year."-".$_POST['select_month']."-".$_POST['select_date'];

$sql = "select * from budget_main where budget_year='$year_active_result[budget_year]' and type_id='$result_ex_bud[type_id]' and status!='null' and rec_date<='$date_index' order by rec_date, id";
}
			$dbquery = mysqli_query($connect,$sql);
			While ($result = mysqli_fetch_array($dbquery))
				{
...
echo "<font  color='#006666' size='3'>$_SESSION[office_name]</font>";
echo "</div>";
...
echo "<Tr><Td align='right'>วันที่&nbsp;&nbsp;<Select  name='select_date'  size='1'>";
...
echo  "<table width='95%' border='0' align='center'>";
 ...
 echo "<tr>
    <td  bgcolor='#FFCCCC'>เงินงบประมาณ</td>
    <td></td>
    <td></td>
    <td></td>
	<td></td>
  </tr>";
  $cash_ar[200]=number_format($cash_ar[200],2);
  $bank_ar[200]=number_format($bank_ar[200],2);
  $office_ar[200]=number_format($office_ar[200],2);
  $sum_type[200]=number_format($sum_type[200],2);
 echo "<tr>
    <td  bgcolor='#FFFFFF'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;เงินงบประมาณ</td>
    <td align='right'>$cash_ar[200]</td>
    <td align='right'>$bank_ar[200]</td>
    <td align='right'>$office_ar[200]</td>
	<td align='right'>$sum_type[200]</td>
  </tr>";
 echo "<tr>
