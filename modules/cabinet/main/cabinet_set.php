...
echo "<Tr><Td align='right'><span id='group'>กลุ่ม</div>&nbsp;&nbsp;</Td>";
echo "<td>";
echo "<div id='workgroup' align='left'><Select  name='workgroup'  id='workgroup'  size='1'>";
echo  "<option  value = ''>เลือก</option>" ;
...
$sql = "insert into cabinet_cabinet (cabinet_id, cabinet_type, cabinet_name, workgroup, cabinet_size, tray_size, cabinet_person) values ( '$_POST[cabinet_id]', '$_POST[cabinet_type]', '$_POST[cabinet_name]', '$_POST[workgroup]', '$_POST[cabinet_size]',  '$_POST[tray_size]',  '$_POST[cabinet_person]')";
$dbquery = mysqli_query($connect,$sql);
}
...
While ($result_workgroup = mysqli_fetch_array($dbquery_workgroup))
   {
		$workgroup = $result_workgroup['workgroup'];
		$workgroup_desc = $result_workgroup['workgroup_desc'];
				if($result_workgroup[workgroup]==$ref_result[workgroup]){
...
$sql = "select cabinet_cabinet.id, cabinet_cabinet.cabinet_id, cabinet_cabinet.cabinet_type, cabinet_cabinet.cabinet_name, cabinet_cabinet.workgroup, cabinet_cabinet.cabinet_size, cabinet_cabinet.tray_size, person_main.prename, person_main.name, person_main.surname  from cabinet_cabinet  left join person_main on cabinet_cabinet.cabinet_person=person_main.person_id order by cabinet_id  limit $start,$pagelen";
$dbquery = mysqli_query($connect,$sql);
echo  "<table width='95%' border='0' align='center'>";
echo "<Tr><Td colspan='9' align='left'><INPUT TYPE='button' name='smb' value='เพิ่มตู้เอกสาร' onclick='location.href=\"?option=cabinet&task=main/cabinet_set&index=1\"'>";
echo "</Td></Tr>";
echo "<Tr bgcolor=#FFCCCC align='center' ><Td width='100'>เลขที่ตู้</Td><Td>ชื่อตู้เอกสาร</Td><Td>กลุ่ม</Td><Td width='150'>ประเภทตู้เอกสาร</Td><Td width='100'>ขนาดตู้(MB)</Td><Td width='100'>ขนาดลิ้นชัก(MB)</Td><Td width='200'>ผู้จัดการ</Td><Td width='50'>ลบ</Td><Td width='50'>แก้ไข</Td></Tr>";
...
$cabinet_type="ตู้เอกส่วนบุคคล";
		...
		$sql_workgroup = "select  * from system_workgroup  where workgroup='$result[workgroup]' ";
		$dbquery_workgroup = mysqli_query($connect,$sql_workgroup);
		$result_workgroup = mysqli_fetch_array($dbquery_workgroup);
		if(	$result ['cabinet_type']==2){
		echo "<td align='left'>ทุกกลุ่ม (คนละ 1 ลิ้นชัก)</td>";
		}
		else{
		echo "<td align='left'>$result_workgroup[workgroup_desc]</td>";
		}
echo "<td align='left'>$cabinet_type</td><Td align='center'>$result[cabinet_size]</Td><Td align='center'>$result[tray_size]</Td> <Td align='left'>$result[prename]$result[name]&nbsp;$result[surname]</Td><Td><div align='center'><a href=?option=cabinet&task=main/cabinet_set&index=2&id=$id&page=$page><img src=images/drop.png border='0' alt='ลบ'></a></div></Td>
...
function e_show(){
		...
		document.getElementById("cabinet_person").value="<?php $user?>";
		}
		else if(document.getElementById("cabinet_type").value=="1"){
		document.getElementById("workgroup").style.display="block";
...
function goto_url(val){
	if(val==0){
		...
		}else if(frm1.cabinet_type.value==""){
			alert("กรุณาเลือกประเภทตู้เอกสาร");
		}else if(frm1.cabinet_name.value==""){
			alert("กรุณากรอกชื่อตู้เอกสาร");
		}else if((frm1.cabinet_type.value=="1") && (frm1.workgroup.value=="")) {
