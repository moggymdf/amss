...
echo "<tr align='center'><td><font color='#006666'><strong>ตู้เอกสารกลาง</strong></font></td></tr>";
echo "</table>";
}
//ส่วนเพิ่มข้อมูล
if($index==1){
...
$sql = "select cabinet_main.id, cabinet_main.doc_subject, cabinet_main.doc_type, cabinet_main.doc_size,  cabinet_main.doc_name, cabinet_main.person_id, cabinet_main.rec_date, person_main.name, person_main.surname , person_main.department from cabinet_main left join person_main on cabinet_main.person_id=person_main.person_id  where  cabinet_main.cabinet_type='1' and  cabinet_main.cabinet_id='$_REQUEST[cabinet_id]'  and cabinet_main.tray_id='$_REQUEST[tray_id]'  and cabinet_main.file_id='$_REQUEST[file_id]' order by id limit $start,$pagelen";
$dbquery = mysqli_query($connect,$sql);
echo  "<table width='100%' border='0' align='center'>";

echo "<Tr ><Td colspan='4' align='left'>";
			//หากลุ่ม
			$sql_cabinet = "select  workgroup from  cabinet_cabinet where  cabinet_id='$_REQUEST[cabinet_id]' ";
			$dbquery_cabinet = mysqli_query($connect,$sql_cabinet);
			$result_cabinet = mysqli_fetch_array($dbquery_cabinet);
			//หาสิทธ์การเก็บเอกสาร
			$sql_person = "select  name  from  person_main  where  department='$result_cabinet[workgroup]' and person_id='$user' ";
			$dbquery_person = mysqli_query($connect,$sql_person);
			$num_person=mysqli_num_rows($dbquery_person);
			if(	$num_person>=1){
			//echo "<INPUT TYPE='button' name='smb' value='เพิ่มเอกสารในแฟ้ม$result_file[file_name]' onclick='location.href=\"?option=cabinet&task=main/ctr_document_mobile&cabinet_id=$_REQUEST[cabinet_id]&tray_id=$_REQUEST[tray_id]&file_id=$_REQUEST[file_id]&add_index=1&page=$_REQUEST[main_page]\"'>";
			}
echo "<Td colspan='3' align='right'><INPUT TYPE='button' name='smb' value='<<กลับไปตู้เอกสาร' onclick='location.href=\"?option=cabinet&task=main/ctr_document_mobile&page=$_REQUEST[main_page]\"'></Td></Tr>";
...
if($totalpages>5){
			...
			echo " <select onchange=\"location.href=this.options

[this.selectedIndex].value;\" size=\"1\" name=\"select\">";
			echo "<option  value=\"\">หน้า</option>";
				for($p=1;$p<=$totalpages;$p++){
				echo "<option  value=\"?$url_link&page=$p\">$p</option>";
				}
...
$sql = "select cabinet_cabinet.id, cabinet_cabinet.cabinet_id, cabinet_cabinet.cabinet_name, cabinet_cabinet.workgroup, cabinet_cabinet.cabinet_size, cabinet_cabinet.tray_size, cabinet_cabinet.cabinet_person, person_main.prename, person_main.name, person_main.surname  from cabinet_cabinet  left join person_main on cabinet_cabinet.cabinet_person=person_main.person_id  where  cabinet_cabinet.cabinet_type='1' order by cabinet_id  limit $start,$pagelen";
$dbquery = mysqli_query($connect,$sql);
echo  "<table width='100%' border='0' align='center'>";
...
echo "<Td></Td><Td></Td><Td></Td></Tr>";
			...
			////หาสิทธ์เก็บเอกสาร
			$sql_person = "select  name  from  person_main  where  department='$result[workgroup]' and person_id='$user' ";
			$dbquery_person = mysqli_query($connect,$sql_person);
			$num_person=mysqli_num_rows($dbquery_person);
			////
							$F++;
