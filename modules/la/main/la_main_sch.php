...
echo  "</tr>";
echo "<Tr align='left'><Td align='right'><span style='display:none'>ไม่ต้องผ่านผู้บังคับบัญชาขั้นต้น&nbsp;&nbsp;</span></Td><Td><span style='display:none'><input type='checkbox'  name='no_comment' id='no_comment' value='1' checked>&nbsp;&nbsp;</span></Td></Tr>";
echo "<Tr align='left'><Td align='right'>ผู้อนุมัติ &nbsp;&nbsp;</Td><Td><Select  name='grant_p_selected'  size='1'>";
...
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=la&task=main/la_main_sch&index=3&id=$_GET[id]&page=$_REQUEST[page]\"'>
		&nbsp;&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=la&task=main/la_main_sch&page=$_REQUEST[page]\"'";
echo "</td></tr></table>";
}
...
echo "<Tr ><Td colspan='2' align='right'><INPUT TYPE='button' name='smb' value='<<กลับหน้าก่อน' onclick='location.href=\"?option=la&task=main/la_main_sch&page=$_GET[page]\"'></Td></Tr>";
$sql = "select * from la_main where id='$_GET[id]'";
$dbquery = mysqli_query($connect,$sql);
$ref_result = mysqli_fetch_array($dbquery);
$id=$ref_result['id'];
...
$no_comment_select="";
		...
		echo "<Tr align='left'><Td align='right'>ผู้อนุมัติ&nbsp;&nbsp;</Td><Td><Select  name='grant_p_selected'  size='1'>";
		echo  "<option  value = ''>เลือก</option>" ;
		$sql = "select * from person_main where person_id='$ref_result[grant_p_selected]' ";
		$dbquery = mysqli_query($connect,$sql);
		While ($result = mysqli_fetch_array($dbquery))
		   {
				$person_id = $result['person_id'];
...
$url_link="option=la&task=main/la_main_sch";  // 2_กำหนดลิงค์ฺ
...
echo "<Tr><Td colspan='11' align='left'><INPUT TYPE='button' name='smb' value='ขออนุญาตลาป่วย ลากิจ ลาคลอด' onclick='location.href=\"?option=la&task=main/la_main_sch&index=1\"'></Td></Tr>";
...
echo "</Td>";
echo "<Td valign='top' align='center'><a href=?option=la&task=main/la_main_sch&index=7&id=$id&page=$page><img src=images/browse.png border='0' alt='รายละเอียด'></Td>";
if(($officer_sign=="") and ($group_sign=="") and ($commander_sign=="")){
echo "<Td valign='top' align='center'><a href=?option=la&task=main/la_main_sch&index=2&id=$id&page=$page><img src=images/drop.png border='0' alt='ลบ'></Td><Td valign='top'  align='center'><a href=?option=la&task=main/la_main_sch&index=5&id=$id&page=$page><img src=images/edit.png border='0' alt='แก้ไข'></a></Td>";
}
else{
echo "<td></td><td></td>";
}
...
function goto_url(val){
	if(val==0){
		callfrm("?option=la&task=main/la_main_sch");   // page ย้อนกลับ
	}else if(val==1){
		...
		alert("กรุณาระบุสาเหตุ");
			...
			callfrm("?option=la&task=main/la_main_sch&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url2(val){
	if(val==0){
		...
		if(frm1.write_at.value == ""){
			...
			callfrm("?option=la&task=main/la_main_sch&index=4.1");   //page ประมวลผล
		}
	}
}
...
function goto_url_update(val){
	if(val==0){
		...
		if(frm1.write_at.value == ""){
			...
			callfrm("?option=la&task=main/la_main_sch&index=6");   //page ประมวลผล
		}
	}
}
