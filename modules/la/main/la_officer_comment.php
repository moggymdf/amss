...
$position_name=$result['position_name'];
		...
		if(!$result_person){
		$sql_person = "select * from  person_sch_main left join system_school on person_sch_main.school_code=system_school.school_code where person_id='$ref_result[person_id]'";
		$dbquery_person = mysqli_query($connect,$sql_person);
		$result_person = mysqli_fetch_array($dbquery_person);
...
echo  "<option  value = ''>เลือก</option>" ;
$sql = "select * from person_main where  position_code>'1' order by department, position_code,person_order";
$dbquery = mysqli_query($connect,$sql);
While ($result = mysqli_fetch_array($dbquery))
   {
		$person_id = $result['person_id'];
...
echo "<Tr bgcolor='#FFCCCC' align='center'><Td width='60'>เลขที่</Td><Td>ผู้ขออนุญาต</Td><Td width='100'>วันขออนุญาต</Td><Td width='100'>ประเภทการลา</Td><Td>ตั้งแต่วันที่ี</Td><Td>ถึงวันที่</Td><Td width='80'>มีกำหนด</Td><Td width='50'>เอกสาร</Td><Td width='60'>รายละเอียด</Td><Td colspan='2'>ตรวจสอบ</Td></Tr>";
...
$fullname=$result_person['prename'].$result_person['name']." ".$result_person['surname'];
		if(!$result_person){
		$sql_person = "select * from  person_sch_main left join system_school on person_sch_main.school_code=system_school.school_code where person_id='$result[person_id]'";
		$dbquery_person = mysqli_query($connect,$sql_person);
		$result_person = mysqli_fetch_array($dbquery_person);
		$fullname=$result_person['prename'].$result_person['name']." ".$result_person['surname']." "."(".$result_person['school_name'].")";
		}

echo "<Tr bgcolor='$color'><Td valign='top' align='center'>$id</Td><Td valign='top' align='left'>$fullname</Td><Td valign='top' align='left'>";
