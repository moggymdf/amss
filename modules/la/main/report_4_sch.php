...
$sql = "select * from person_sch_main left join system_school on person_sch_main.school_code=system_school.school_code where person_sch_main.status='0' and person_sch_main.position_code='1' and system_school.school_type='1' order by system_school.school_code";
$dbquery = mysqli_query($connect,$sql);

echo "<Tr bgcolor='#FFCCCC' align='center'><Td width='50' rowspan='2'>ที่</Td>";
echo "<Td rowspan='2'>โรงเรียน</Td><Td rowspan='2'>ชื่อ</Td><Td rowspan='2'>ตำแหน่ง</Td><Td colspan='2'>ลาป่วย</Td><Td colspan='2'>ลากิจ</Td><Td colspan='2'>ลาคลอด</Td></Tr>";
...
$kod_day_cancel=0;
			...
			if($_SESSION['login_user_id']==$person_id){
			echo "<Td align='left'><a href=?option=la&task=main/report_6_sch&person_id=$person_id>$prename&nbsp;$name&nbsp;&nbsp;$surname</a></Td><Td align='left'>";
			}
			else{
...
callfrm("?option=la&task=main/report_4_sch");
}
</script>
