...
echo "<Tr ><Td colspan='2' align='right'><INPUT TYPE='button' name='smb' value='<<กลับหน้าก่อน' onclick='location.href=\"?option=la&task=main/report_6_sch&page=$_GET[page]&person_id=$user\"'></Td></Tr>";
...
echo "</Td></Tr>";
		...
		$sql_name = "select * from person_sch_main left join person_sch_position on person_sch_main.position_code=person_sch_position.position_code where person_sch_main.person_id='$ref_result[person_id]'";
		$dbquery_name = mysqli_query($connect,$sql_name);
		$result_name = mysqli_fetch_array($dbquery_name);
...
$url_link="option=la&task=main/report_6_sch&person_id=$_GET[person_id]";  // 2_กำหนดลิงค์ฺ
...
echo "<Tr ><Td align='right'><INPUT TYPE='button' name='smb' value='<<กลับหน้าสถิติการลา' onclick='location.href=\"?option=la&task=main/report_4_sch\"'></Td></Tr>";
echo "</table>";
...
echo "</Td>";
echo "<Td valign='top' align='center'><a href=?option=la&task=main/report_6_sch&index=7&id=$id&person_id=$user&page=$page><img src=images/browse.png border='0' alt='รายละเอียด'></Td>";
echo "</Tr>";
...
function goto_url(val){
	if(val==0){
		callfrm("?option=la&task=main/report_6_sch");   // page ย้อนกลับ
	}else if(val==1){
		...
		alert("กรุณาระบุสาเหตุ");
			...
			callfrm("?option=la&task=main/report_6_sch&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url2(val){
	if(val==0){
		...
		if(frm1.write_at.value == ""){
			...
			callfrm("?option=la&task=main/report_6_sch&index=4.1");   //page ประมวลผล
		}
	}
}
...
function goto_url_update(val){
	if(val==0){
		...
		if(frm1.write_at.value == ""){
			...
			callfrm("?option=la&task=main/report_6_sch&index=6");   //page ประมวลผล
		}
	}
}
