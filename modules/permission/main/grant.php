...
echo "<Tr align='left'><Td align='right'>การอนุมัติ&nbsp;&nbsp;</Td><Td><Input Type='radio' Name='grant' id='grant' value='1' $grant_1_checked>อนุมัติ</Td></Tr>";
...
if($ref_result['report']!=""){
echo "<table width='500'><tr><td>";
echo "<fieldset>";
echo "<legend>&nbsp;<B>ส่วนของรายงาน</B>: &nbsp;</legend>";
echo "<table width='700'>";
$thai_date_report=thai_date_4($ref_result['report_date']);
echo "<Tr align='left'><Td valign='top' align='left' colspan='2'>$ref_result[report]</Td></Tr>";
...
echo "</fieldset></td></tr></table>";
			...
			//บันทึกอ่านรายงาน
			if(isset($_GET['report_read'])){
				if($ref_result['grant_person']==$_SESSION['login_user_id']){
						if($ref_result['report_read']==""){
						$date_time_now = date("Y-m-d H:i:s");
						$sql = "update permission_main set report_read='$_SESSION[login_user_id]',report_read_date='$date_time_now' where id='$_GET[id]'";
						$dbquery = mysqli_query($connect,$sql);
						}
				}
			}
...
$sql = "select permission_main.id, permission_main.person_id, permission_main.subject, permission_main.place, permission_main.vehicle, permission_main.ref_id, permission_main.document, permission_main.comment, permission_main.grant_x, permission_main.grant_comment,permission_main.grant_date, permission_main.grant_person_selected, permission_main.rec_date,permission_main.report ,permission_main.report_read from permission_main left join permission_person_set on  permission_main.person_id=permission_person_set.person_id where (permission_main.grant_person_selected ='$_SESSION[login_user_id]' or permission_person_set.grant_person ='$_SESSION[login_user_id]') and (permission_main.rec_date<'$onedayago' or permission_person_set.comment_person is null or permission_person_set.comment_person='' or permission_main.no_comment='1' or permission_main.comment_person is not null) order by permission_main.grant_x desc,permission_main.id limit $start,$pagelen";

$dbquery = mysqli_query($connect,$sql);

echo "<form id='frm1' name='frm1'>";
echo  "<table width='98%' border='0' align='center'>";
echo "<Tr><Td colspan='11' align='right'><INPUT TYPE='checkbox' name='allchk'  id='allckk' onclick='CheckAll()'>เลือก/ไม่เลือกทั้งหมด</Td><td></td></Tr>";
echo "<Tr bgcolor='#FFCCCC' align='center'><Td width='60'>เลขที่</Td><Td width='120'>ผู้ขออนุญาต</Td><Td width='90'>วันขออนุญาต</Td><Td>เรื่องราชการ</Td><Td>สถานที่</Td><Td width='100'>วันไปราชการ</Td><Td width='100'>พาหนะ</Td><Td width='50'>เอกสาร</Td><Td width='50'>รายละเอียด</Td><Td width='120'><INPUT TYPE='button' name='smb' value='อนุมัติไปราชการ' onclick='goto_url(1)'></Td><Td width='40'>บันทึก</Td><Td width='100'>รายงาน</Td></Tr>";
...
if($result['report']!=""){
echo "<a href=?option=permission&task=main/grant&index=7&id=$id&page=$page&report_read=1>รายงาน</a>";
		if($result['report_read']!=""){
		echo "<font size='1'>[อ่านแล้ว]</font>";
		}
}
