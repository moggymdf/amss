...
echo "<Tr ><Td colspan='2' align='right'><INPUT TYPE='button' name='smb' value='<<กลับหน้าก่อน' onclick='location.href=\"?option=plan&task=check/receive_report&page=$_GET[page]\"'></Td></Tr>";
...
$url_link="option=plan&task=check/receive_report";  //2.กำหนด url เพจ
...
While ($result = mysqli_fetch_array($dbquery))
	{
		...
		<Td><div align=center><a href=?option=plan&task=check/receive_report&index=7&id=$id&page=$page><img src=./images/browse.png border='0' alt='รายละเอียด'></a></td>";
	//กำหนดสัญญาลักษ์ ถึงนี้
		...
		if($_GET['cal_id']==$id){
		echo "<Td align='center'><a href=?option=plan&task=check/receive_report&page=$page>$tungnee</a></Td>";
		echo "</Tr>";
		break;  //ออกจากloop
		}
		else {
		echo "<Td align='center'><a href=?option=plan&task=check/receive_report&cal_id=$id&page=$page>ถึงนี้</a></Td>";
		echo "</Tr>";
		}
$M++;
