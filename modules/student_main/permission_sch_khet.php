...
$(function(){
	$("select#school_code").change(function(){
		var datalist2 = $.ajax({	// รับค่าจาก ajax เก็บไว้ที่ตัวแปร datalist2
			  url: "modules/student_main/return_permission_sch_khet.php", // ไฟล์สำหรับการกำหนดเงื่อนไข
...
echo "<tr align='center'><td><font color='#006666' size='3'><strong>เจ้าหน้าที่ข้อมูลพื้นฐานนักเรียนของโรงเรียน</strong></font></td></tr>";
echo "</table>";
}
...
echo "<Center>";
echo "<Font color='#006666' Size=3><B>เพิ่มเจ้าหน้าที่ข้อมูลนักเรียนให้สถานศึกษา</Font>";
echo "</Cener>";
echo "<Br><Br>";
...
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=student_main&task=permission_sch_khet&index=3&id=$_GET[id]&page=$_REQUEST[page]\"'>
		&nbsp;&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=student_main&task=permission_sch_khet&page=$_REQUEST[page]\"'";
echo "</td></tr></table>";
}
...
if($index==4){
$rec_date = date("Y-m-d");
$sql = "insert into student_main_permission (person_id, school_code, p2, officer,rec_date) values ('$_POST[person_id]', '$_POST[school_code]', '1','$_SESSION[login_user_id]','$rec_date')";
$dbquery = mysqli_query($connect,$sql);
}
...
if ($index==6){
$rec_date = date("Y-m-d");
$sql = "update student_main_permission set  person_id='$_POST[person_id]', school_code='$_POST[school_code]',  officer='$_SESSION[login_user_id]', rec_date='$rec_date' where id='$_POST[id]'";
$dbquery = mysqli_query($connect,$sql);
}
...
$url_link="option=student_main&task=permission_sch_khet";  // 2_กำหนดลิงค์ฺ
$sql = "select student_main_permission.id from student_main_permission where student_main_permission.p2='1' ";
$dbquery = mysqli_query($connect,$sql);
...
$sql = "select student_main_permission.id,  student_main_permission.officer, person_sch_main.prename,person_sch_main.name, person_sch_main.surname,student_main_permission.school_code from student_main_permission left join person_sch_main on student_main_permission.person_id=person_sch_main.person_id  where student_main_permission.p2='1' order by student_main_permission.school_code, student_main_permission.id  limit $start,$pagelen";
$dbquery = mysqli_query($connect,$sql);
echo  "<table width=70% border=0 align=center>";
echo "<Tr><Td colspan='5' align='left'><INPUT TYPE='button' name='smb' value='เพิ่มเจ้าหน้าที่' onclick='location.href=\"?option=student_main&task=permission_sch_khet&index=1\"'</Td></Tr>";
...
While ($result = mysqli_fetch_array($dbquery))
	{
		...
		if($person_level==1){
		echo "<Td align='center' width='50' ><a href=?option=student_main&task=permission_sch_khet&index=2&id=$id&page=$page><img src=images/drop.png border='0' alt='ลบ'></a></Td>
		<Td align='center' width='50'><a href=?option=student_main&task=permission_sch_khet&index=5&id=$id&page=$page><img src=images/edit.png border='0' alt='แก้ไข'></a></Td>";
		}
		else{
		echo "<td></td><td></td>";
		}
...
function goto_url(val){
	if(val==0){
		callfrm("?option=student_main&task=permission_sch_khet");   // page ย้อนกลับ
	}else if(val==1){
		if(frm1.school_code.value == ""){
			...
			callfrm("?option=student_main&task=permission_sch_khet&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url_update(val){
	if(val==0){
		...
		if(frm1.school_code.value == ""){
			...
			callfrm("?option=student_main&task=permission_sch_khet&index=6");   //page ประมวลผล
		}
	}
}
</script>
