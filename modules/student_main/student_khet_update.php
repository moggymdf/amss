...
echo "<Table width='60%' Border='0'>";
	...
	//เลือก	โรงเรียน
echo "<Tr align='left'><Td ></Td><Td align='right'>โรงเรียน&nbsp;&nbsp;&nbsp;&nbsp;</Td><Td>";
...
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=student_main&task=student_khet_update&index=3&id=$_GET[id]&class_index=$_REQUEST[class_index]&page=$_REQUEST[page]&school_index=$_REQUEST[school_index]\"'>
		&nbsp;&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=student_main&task=student_khet_update&class_index=$_REQUEST[class_index]&page=$_REQUEST[page]&school_index=$_REQUEST[school_index]\"'";
echo "</td></tr></table>";
}

if($index==2.1) {
echo "<table width='90%' border='0' align='center'>";
echo "<tr><td align='center'><font color='#990000' size='4'>จะทำการลบข้อมูลทั้งหมด ทุกหน้า<br>โปรดยืนยันความต้องการลบข้อมูลอีกครั้ง</font><br></td></tr>";
echo "<tr><td align=center>";
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=student_main&task=student_khet_update&index=3.1\"'>
		&nbsp;&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=student_main&task=student_khet_update\"'";
echo "</td></tr></table>";
}
...
if($index==3.1){
$sql = "delete from student_main_main where ed_year='$year_active_result[ed_year]' ";
$dbquery = mysqli_query($connect,$sql);
}
...
$rec_date = date("Y-m-d");
	...
	$sql = "select * from student_main_main where  student_id='$_POST[student_id]' and ed_year='$year_active_result[ed_year]' and school_code='$_POST[school_index]'";
	$dbquery = mysqli_query($connect,$sql);
...
$ref_id=$_POST['school_index'].$_POST['student_id'];

		$sql_school = "select * from  system_school  where school_code='$_POST[school_index]' ";
		$dbquery_school = mysqli_query($connect,$sql_school);
		$result_school = mysqli_fetch_array($dbquery_school);


$sql = "insert into student_main_main (ed_year,ref_id,school_code,student_id,person_id,prename,name,surname,sex,school_name,classlevel,classroom,rec_date,officer) values ( '$year_active_result[ed_year]','$ref_id','$_POST[school_index]','$_POST[student_id]','$_POST[person_id]','$_POST[prename]','$_POST[name]','$_POST[surname]','$_POST[sex]','$result_school[school_name]','$_POST[classlevel]','$_POST[classroom]','$rec_date','$officer')";
$dbquery = mysqli_query($connect,$sql);
}
...
echo "<Tr align='left'><Td ></Td><Td align='right'>โรงเรียน&nbsp;&nbsp;&nbsp;&nbsp;</Td><Td>";
			echo "<Select  name='school_index' size='1'>";
			echo  "<option  value = ''>เลือกโรงเรียน</option>" ;
		$sql_school = "select * from  system_school  order by school_type,school_code";
		$dbquery_school = mysqli_query($connect,$sql_school);
		While ($result_school = mysqli_fetch_array($dbquery_school))
		   {
...
if ($index==6){
	$sql = "select * from student_main_main where  student_id='$_POST[student_id]' and ed_year='$year_active_result[ed_year]' and school_code='$_REQUEST[school_index]' and id!='$_POST[id]' ";
	$dbquery = mysqli_query($connect,$sql);
...
$rec_date = date("Y-m-d");
$ref_id=$_REQUEST['school_index'].$_POST['student_id'];
$sql = "update student_main_main set student_id='$_POST[student_id]',
ref_id='$ref_id',
school_code='$_POST[school_index]',
school_name='$result_school[school_name]',
...
$url_link="option=student_main&task=student_khet_update&year_index=$_REQUEST[year_index]&class_index=$_REQUEST[class_index]&school_index=$_REQUEST[school_index]";  // 2_กำหนดลิงค์ฺ
...
//จบแยกหน้า
 		...
 		//เลือกชั้น
		echo  "<table width=90% border='0' align='center'>";
		echo "<Tr><td align='left'><INPUT TYPE='button' name='smb' value='เพิ่มข้อมูล' onclick='location.href=\"?option=student_main&task=student_khet_update&index=1\"'><INPUT TYPE='button' name='smb' value='ลบข้อมูลทั้งหมด' onclick='location.href=\"?option=student_main&task=student_khet_update&index=2.1\"'></td>";
		echo "<td align='right'>";
		echo "<form id='frm1' name='frm1'>";
...
echo "<td><a href=?option=student_main&task=student_khet_update&index=2&id=$result[id]&class_index=$_REQUEST[class_index]&page=$page&school_index=$_REQUEST[school_index]><img src=images/drop.png border='0' alt='ลบ'></a></td><td><a href=?option=student_main&task=student_khet_update&index=5&id=$result[id]&class_index=$_REQUEST[class_index]&page=$page&school_index=$_REQUEST[school_index]><img src=images/edit.png border='0' alt='แก้ไข'></a></td>";
echo "</Tr>";
$M++;
...
function goto_index(val){
	if(val==1){
		callfrm("?option=student_main&task=student_khet_update");   // page ย้อนกลับ
		}
}
...
function goto_url(val){
	if(val==0){
		...
		if(frm1.school_index.value == ""){
			alert("กรุณาเลือกโรงเรียน");
		}else if(frm1.student_id.value==""){
			...
			callfrm("?option=student_main&task=student_khet_update&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url_update(val){
	if(val==0){
		...
		if(frm1.school_index.value == ""){
			...
			callfrm("?option=student_main&task=student_khet_update&index=6");   //page ประมวลผล
		}
	}
}
